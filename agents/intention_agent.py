...
- 【用户偏好】只能影响推荐排序和表达方式，不得自动成为当前行程的出发地、目的地、日期、时长或出差目的。
...
【Query 改写要求】
将口语化表达标准化；只可根据【当前出差任务】或本会话中用户明确说出的事实补全省略信息（如把“那边”指代回填为本会话已确认的目的地）。不得从用户偏好、历史行程、历史会话摘要或助手说过的话中补全。若无需改写则原样保留用户输入。
            ...
            messages = [
                {
                    ...
                    "content": (
                        ...
                        "用户偏好、历史行程、历史摘要和助手陈述均不得作为当前行程事实。"
                    ),
                },
